...
import { handleCronJob, fetchMiddlewareHandler } from './handler'
import { getDeployedCommonSubgraphUrls, getDeployedAnalyticsSubgraphUrls, getSubgraphHealth } from './manualDeps'
...
export class SubgraphHealthEndpoint {
  async getHealthByUri(
    ...
    | {
        ...
        network: string
        fatalError: { message: string; block: number; handler: any } | undefined
  ...
  async getHealthForAllChains() {
    ...
    for (const chainId of CHAINS_TO_MONITOR) {
      const uris = getDeployedCommonSubgraphUrls(chainId)
...
addEventListener('fetch', async(event) => {
  const res = await fetchMiddlewareHandler(event.request)
